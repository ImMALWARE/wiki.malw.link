...
::: tip Сложный, но лучший вариант
В этой статье для использования предлагаются именно готовые сборки zapret. Сборки - простое решение, которое может помочь большинству, но может и не помочь или создать новые проблемы, именно поэтому сам разработчик оригинального инструмента и [рекомендует подбирать стратегии самостоятельно под свои цели](https://github.com/bol-van/zapret/blob/master/docs/quick_start_windows.md#немного-разъяснений), хотя и без знаний это куда сложнее. Если Вы готовы попробовать свои силы в создании собственной сборки со стратегией, которая будет работать именно так, как нужно Вам, то Вы можете ознакомиться с [этой статьёй](/network/vpns/blockcheck). В ней описан минимальный алгоритм работы с инструментом blockcheck из оригинального репозитория, что может Вам помочь.
:::
