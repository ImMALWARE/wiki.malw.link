...
Нужно выполнить скрипт в bash-терминале. Локальный Linux-компьютер не подойдёт, так как при генерации конфига делаются запросы на сервера, которые заблокированы. Поэтому лучше использовать бесплатные онлайн-терминалы:
