...
Android позволяет установить дополнительные сертификаты без root. Но приложения по умолчанию доверяют только системным сертификатам, соединения проходить не будут. Поэтому есть [Magisk](/android/root) модуль **Cert Fixer**, который копирует пользовательские сертификаты в системное хранилище. Тогда все приложения будут доверять сертификату PCAPdroid, соединения будут успешны, PCAPdroid mitm сможет их расшифровать.
...
5. В разделе "Шифрование и учетные данные" будет кнопка "Установка сертификатов".
    <img src="/img/apps/pcapdroid/install-ca.png" class="centered">
