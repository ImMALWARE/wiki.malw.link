...
html:not(.dark) .landscape .landscapeItem {
	filter: invert(1);
}
...
html:not(.dark) .landscape .landscapeItem.mountains.background.animateIn {
	opacity: 0.15;
...
.landscape .landscapeItem.mountains.foreground {
	z-index: 3;
	background-image: url(/img/mountains/3.svg);
}
...
.landscape .landscapeItem.trees.midground {
	z-index: 5;
	background-image: url(/img/mountains/1.svg);
}
.landscape .landscapeItem.trees.foreground {
	z-index: 6;
	background-image: url(/img/mountains/0.svg);
}
...
onMounted(() => {
	...
	const animate = (): void => {
		...
		backgroundContext.fillStyle = isDark ? '#000' : '#fff';
		backgroundContext.fillRect(0, 0, width, height);
		backgroundContext.fillStyle = isDark ? '#fff' : '#000';
		backgroundContext.strokeStyle = isDark ? '#fff' : '#000';
